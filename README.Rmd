...
app provides dedicated APA 7 plain-text, HTML, and BibTeX copy buttons. GitHub
does not allow clipboard scripts inside a README, so the compact controls below
open a code block with GitHub's native copy button.

<details>
<summary>Copy plain text</summary>
...
measures: $r_\mathrm{es}$, $r_\mathrm{contrast}$, and
$r_\mathrm{alerting}$. A pure within-subjects analysis instead reports
$r_\mathrm{contrast}$ and $g_\mathrm{contrast}$ because the other two
denominators require a between-group partition.
...
For uploaded data, cofad automatically examines replication and nesting to
...
Click the first screenshot to open the server-backed app. The browser-only webR
version is available at <https://johannes-titz.github.io/cofad/>.
...
You can see that the significance test is a one-tailed $t$-test and the paper-ready report gives the signed effect-size correlation $r_\mathrm{contrast}$. When conducting the analysis by hand, we can see why:
...
$\eta^2$ and partial $\eta_p^2$ shown in the app. The between-design
...
We will again look at the example presented in @rosenthal2000 (see the
graphical-user-interface section). The cognitive ability of nine children
belonging to different age groups (between) was measured four times (within).
...
The results look like a between-subjects contrast analysis. The summary adds
the effect sizes and the between-group means and standard errors of the selected
...
browser process. As of August 2026, line coverage is 93.33% overall. Browser
