...
As an example go to https://cofad.titz.science/example which will load a data set from @rosenthal2000 (Table 5.3). Nine children were  measured four times (within), but they also belong to different groups of age (between).
...
You can see that the significance test is just a $t$-test and the reported effect size is also for a mean comparison ($g$). (The $t$-test is one-tailed, because contrast analysis has always a specific hypotheses.) When conducting the analysis manually, we can see why:
