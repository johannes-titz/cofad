...
The idea of mixed designs is a combination of between and within factors. In this case cofad calculates the *L-Value* for the within factor, which it then treats as a new dependent variable. @Rosenthal2000, in their table 5.3, give an illustrative example: Nine children are measured four times (within), but they also belong to different groups of age (between).
...
contr_mx
```
The results look like a contrast analysis for between-subject designs, the second step of the mixed desing contrast analysis. Again, the summary gives some more details: The effect sizes and within group means and standard errors of the L-Values. 
```{r}
summary(contr_mx)
```
