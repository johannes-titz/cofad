...
Sometimes you would like to run a contrast analysis on aggregated data. If you have the means, standard deviations and sample sizes for every condition, you can do this in cofad.
...
d_agg <- d %>% group_by(major) %>% summarize(mean = mean(empathy),
                                             sd = sd(empathy), n = n())

calc_contrast_aggregated(d_agg$mean, d_agg$sd, d_agg$n, lambdas = c(0, 0, -1, 1))
```
Let us compare this with the raw data analysis:
